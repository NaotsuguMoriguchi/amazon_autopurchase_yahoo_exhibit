...
const exhibit_Main = async (store_id, access_token) => {
	...
	await exSettingList.findOne({ where: { user_id: Number(user_id) } })
		.then(setting_list => {
				...
				.then(exhibit_items => {
					...
					let exInterval = setInterval(async () => {
						if (index < len) {
							...
							if (setting_list.not_asin != null) {
								for (let a = 0, not_asins = setting_list.not_asin, len = not_asins.length; a < len; a++) {
									if (not_asins[a] == i.asin) {
										flag = false;
										return;
									}
								}
							...
							if (setting_list.not_word != null && flag == true) {
								for (let a = 0, not_words = setting_list.not_word, len = not_words.length; a < len; a++) {
							...
							if (flag == true) {
								...
								if (setting_list.remove_word != null) {
									for (let a = 0, remove_words = setting_list.remove_word, len = remove_words.length; a < len; a++) {
										if (i.name.includes(not_words[a])) {
											flag = false;
											return;
										}
								...
								if (setting_list.price_settings.length != 0 && JSON.parse(setting_list.price_settings).length != 0) {

									var price_set = JSON.parse(setting_list.price_settings);

									for (let p = 0; p < price_set.length; p++) {
										var amazonP = i.am_price;
										var startP = Number(price_set[p].start_price);
										var endP = Number(price_set[p].end_price);
										var plusA = Number(price_set[p].plus_amount);
										var minusA = Number(price_set[p].minus_amount);
										var profitR = Number(price_set[p].profit_rate);
										var profitA = Number(price_set[p].profit_amount);
								...
								var yahoo_category = JSON.parse(setting_list.yahoo_setting).category;
								console.log(JSON.parse(setting_list.yahoo_setting));
								...
								var save_info = {
									...
									'caption': i.caption,
									'delivery': i.delivery,
									'lead_time_instock': i.date_info,
									'lead_time_outstock': i.date_info_out,
									'postage_set': i.deli_group,
									'sp_additional': i.sp_additional??'',
									'sp_additional': i.sp_additional??'',
								};
