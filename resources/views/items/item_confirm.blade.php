		...
		<div class="pagetitle">
			<nav>
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active">出品済みデータ確認</li>
				</ol>
			</nav>
		</div><!-- End Page Title -->
		<div class="card">
...
<script>
	...
	var datatable = $('.datatable').DataTable({
		...
		pageLength: 10,
		ajax: {
			'url': "{{ route('item_exhibit_datatable') }}",
			'data': function (d) {
		...
		columns: [
			...
			// },
			{
				...
				render:function(data, type ,row) {
					return (
						`<img src=${row.img_url} style="width: 64px; height: 64px;" />`
					)
				}
			},
			{
			...
			// 	render: function(data, type, row) {
			// 		return (
			// 			`<span>${row.updated_at.replace("T", " ").replace("000000Z", "")}</span>`
			// 		)
			// 	}
			// }
		]
	...
	function exhibit () {
		...
		// let clientId = "dj00aiZpPXpkb1NFblhsU29KTSZzPWNvbnN1bWVyc2VjcmV0Jng9ZDg-";
