	...
	<div class="card-body">
		...
		<form id="formAuthentication" class="mb-3" method="POST" action="{{ route('login') }}">
			...
			<div class="mb-3">
				...
				<input type="email" class="form-control" id="email" name="email" placeholder="Emailを入力してください。" autofocus />
			</div>
			@error('email')
			<small class="text-danger text-xs">メールアドレスを確認してください。</small>
			@enderror
			<div class="mb-3 form-password-toggle">
				<div class="d-flex justify-content-between">
					<label class="form-label" for="password">パスワード</label>
					<!-- <a href="{{ route('password.request') }}">{{__('messages.auth.forget_password')}}</a> -->
				</div>
				<div class="input-group input-group-merge">
					<input type="password" id="password" class="form-control" name="password" placeholder="{{__('messages.profile.password')}}を入力してください。" aria-describedby="password" />
					<span class="input-group-text cursor-pointer"><i class="bx bx-hide"></i></span>
				</div>
				@error('password')
				<small class="text-danger text-xs">パスワードを確認してください。</small>
				@enderror
			</div>
			<!-- <div class="mb-3">
