...
<div class="card auth_card">
	<div class="card-body">
		...
		@endif
		{{ isset($error) ? $error : '' }}
		<form id="formAuthentication" class="mb-3" method="POST" action="{{ route('register') }}" role="form">
		@csrf	
		<div class="mb-2">
			...
			@error('name')
				<small class="text-danger text-xs">お名前を確認してください。</small>                                   
			@enderror
			<div class="mb-2">
			...
			@error('email')
				<small class="text-danger text-xs">メールを確認してください。</small>
			@enderror
			...
			@error('password_confirmation')
				<small class="text-danger text-xs">パスワードが一致しません。</small>
			@enderror
			<!-- <div class="mb-3">
			...
			<button type="submit" class="btn btn-primary d-grid w-100">登録</button>
		</form>

		<p class="text-center">
			<a href="{{  url('/login') }}"><span>すでにアカウントを持っていますか?</span></a>
		</p>
	</div>
</div>
@endsection
